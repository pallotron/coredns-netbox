...
A Helm chart deploying CoreDNS with Netbox-backed DNS zones. A Go sidecar periodically scrapes Netbox for IP addresses, auto-discovers zones (by FQDN depth, common suffix, or via the Netbox DNS plugin API), and writes zone files to a shared volume. CoreDNS serves the zone files using the `auto` plugin and falls through to the [netbox plugin](https://github.com/oz123/coredns-netbox-plugin) for records not yet in the zone files. Zone transfers (AXFR) are supported for replicating zones to secondary DNS servers.
...
**Request flow (primary):** cache → auto (local zone files) → netbox (API fallthrough) → forward (upstream)
...
helm upgrade --install coredns-netbox ./helm/coredns-netbox \
  ...
  --set 'secondary.zones[1]=internal.corp' \
...
**Netbox compatibility:** The DNS client works with both Netbox 3.x and 4.x. It uses raw HTTP against the stable Netbox REST API and auto-detects token type (v1 `Token` auth vs v2 `nbt_` `Bearer` auth). The dev seed tool (`cmd/seed/`) uses the `go-netbox/v4` SDK and requires Netbox 4.x.
...
- **`zone-depth`** (default): Uses the last N labels of each FQDN as the zone name. With `depth=2`, `host1.example.org` becomes zone `example.org`.
...
2. **CoreDNS** serves DNS using local zone files via the `auto` plugin, with fallthrough to the netbox plugin for cache misses
...
```bash
dig @127.0.0.1 -p 15353 host1.example.org A
dig @127.0.0.1 -p 15353 host1.example.org AAAA
dig @127.0.0.1 -p 15353 google.com A  # forwarded

# Secondary (after second `make dev.deploy` to pick up primary ClusterIP)
dig @127.0.0.1 -p 15354 host1.example.org A
```
...
| Target | Description |
|---|---|
| `make build` | Build sidecar and seed binaries |
...
| `make dev.seed` | Populate Netbox with test records |
...
# 4. Seed Netbox with test DNS records
make dev.seed
...
dig @127.0.0.1 -p 15353 host1.example.org A       # primary
dig @127.0.0.1 -p 15354 host1.example.org A       # secondary (AXFR replica)
dig @127.0.0.1 -p 15353 example.org AXFR +tcp     # full zone transfer
```
...
The seed tool (`cmd/seed/`) creates these test records in Netbox:

| DNS Name | Address | Type |
|---|---|---|
| host1.example.org | 10.0.0.1/24 | A |
| host2.example.org | 10.0.0.2/24 | A |
| host3.example.org | 10.0.0.3/24 | A |
| host4.example.org | 10.0.0.4/24 | A |
| host5.example.org | 10.0.0.5/24 | A |
| web.example.org | 192.168.1.1/24 | A |
| db.example.org | 192.168.1.2/24 | A |
| cache.example.org | 192.168.1.3/24 | A |
| host1.example.org | 2001:db8::1/64 | AAAA |
| host2.example.org | 2001:db8::2/64 | AAAA |
